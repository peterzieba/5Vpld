#This is a parser for JEDEC files, taken and modified from whitequark/prjbureau
...
class JESD3Parser:
    ...
    def _on_end(self, checksum):
        ...
        if expected_checksum == 0x0000:
            return
        #actual_checksum   = self._lexer.checksum & 0xffff #the transmission checksum is calculated improperly. We use the value from txchecksum, which opens the .JED file as binary, which doesn't break CRLF.
        actual_checksum=txchecksum
        if expected_checksum != actual_checksum:
...
if __name__ == "__main__":
    ...
    txchecksum=0
    txcheckstate=0
    #This block only hanles calculating the transmission checksum of the .JED file. It is a hack around the main parsers attempt to do this.
    with open(sys.argv[1], 'rb') as fh:
        while (byte := fh.read(1)):
            ascii_dec=int.from_bytes(byte, "little") #Convert each byte read to an ascii integer 0-255

            if ascii_dec==2: #If STX byte is detected, then we are within the valid parts of the file
                if txcheckstate==1:
                    print("WARNING: detected multiple STX")
                if txcheckstate==2:
                    print("WARNING: STX detected after EXT")
                    #print(str(counter))
                txcheckstate=1

            if txcheckstate==1:
                txchecksum+=ascii_dec

            if ascii_dec==3: #If ETX byte is detected, then we're at the end.
                if txcheckstate==0:
                    print("WARNING: EXT encountered before STX")
                if txcheckstate==2:
                    print("WARNING: Another ETX encountered after EXT")
                txcheckstate=2
            #counter+=1

    #This block handles the actual parsing of the .JED file.
